...
public class ControleurBillePiloteeLibre extends ControleurEtatBillePilotee {
    ...
    protected void MousePressed(Vecteur mousePosition) {
        if (Math.sqrt(Math.pow((mousePosition.x - bille.getPosition().x), 2) + Math.pow((mousePosition.y - bille.getPosition().y), 2)) <= bille.getRayon()) {
            this.bille.setCourant(this.suivant);
        }
    }
